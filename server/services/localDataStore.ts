...
class LocalDataStore {
  ...
  public getTickets(filter?: { status?: string; search?: string }): LocalTicket[] {
    ...
    if (filter?.status && filter.status !== 'all') {
      if (filter.status === 'available') {
        list = list.filter(t => t.status === 'available' && !t.buyerName);
      } else if (filter.status === 'registered') {
        list = list.filter(t => t.status === 'registered' || !!t.buyerName);
      } else if (filter.status === 'entered') {
        list = list.filter(t => t.entered);
      } else if (filter.status === 'not-entered') {
        list = list.filter(t => (t.status === 'registered' || !!t.buyerName) && !t.entered);
  ...
  public markEntry(code: string): { success: boolean; ticket?: LocalTicket; message?: string } {
    ...
    t.entered = true;
    t.enteredAt = new Date();
    t.entryCount = (t.entryCount || 0) + 1;
    t.updatedAt = new Date();

    // Update booking entered count if associated
    if (t.bookingId) {
      const b = this.bookings.get(t.bookingId) || Array.from(this.bookings.values()).find(x => x.bookingCode === t.bookingCode);
      if (b && !wasEntered) {
        b.enteredCount = (b.enteredCount || 0) + 1;
  ...
  public markNotEntered(code: string): { success: boolean; ticket?: LocalTicket; message?: string } {
    ...
    if (t.bookingId && wasEntered) {
      const b = this.bookings.get(t.bookingId) || Array.from(this.bookings.values()).find(x => x.bookingCode === t.bookingCode);
      if (b) {
        b.enteredCount = Math.max(0, (b.enteredCount || 0) - 1);
        b.updatedAt = new Date();
      }
    }
  ...
  public cancelTicket(code: string, reason?: string): { success: boolean; ticket?: LocalTicket; message?: string } {
    ...
    t.status = 'cancelled';
    t.cancellationReason = reason || 'DAMAGED';
    t.updatedAt = new Date();
    return { success: true, ticket: t };
  }
  ...
  public clearTicket(code: string): { success: boolean; ticket?: LocalTicket; message?: string } {
    ...
    if (oldBookingCode) {
      ...
      if (b) {
        b.ticketCodes = b.ticketCodes.filter(c => c !== t.code);
        b.ticketQuantity = b.ticketCodes.length;
        b.updatedAt = new Date();
      }
    }
  ...
  public createBooking(data: {
    ...
    amountPaid?: number;
    notes?: string;
  }): { success: boolean; booking?: LocalBooking; tickets?: LocalTicket[]; error?: string } {
    this.initTickets();
    const codes = data.ticketCodes.map(c => c.toUpperCase().trim());

    // Validate codes availability
    ...
    const now = new Date();
    const amount = data.amountPaid ?? data.ticketQuantity * 500;
    ...
    for (const code of codes) {
      const t = this.tickets.get(code)!;
      t.status = 'registered';
      t.buyerName = booking.buyerName;
      t.phone = booking.phone;
      t.email = booking.email || null;
  ...
  public updateBooking(id: string, updates: Partial<LocalBooking>): { success: boolean; booking?: LocalBooking; error?: string } {
    ...
    // Propagate customer updates to associated tickets
    for (const tc of b.ticketCodes) {
      const t = this.tickets.get(tc);
      if (t) {
        t.buyerName = b.buyerName;
        t.phone = b.phone;
        t.email = b.email || null;
        t.updatedAt = new Date();
      }
    }
  ...
  public resetEvent(reason?: string): { success: boolean; backupId: string; message: string } {
    ...
    this.backups.push({
      _id: backupId,
      reason: reason || 'Admin Event Reset',
    ...
    return {
      success: true,
      backupId,
      message: 'All 50 tickets successfully reset to available in local storage.'
    };
  }
  ...
  public getDashboardStats(): any {
    ...
    const todayBookings = bList;
    const todayRevenue = todayBookings.reduce((sum, b) => sum + (b.amountPaid || b.totalAmount || 0), 0);
    const todayCash = todayBookings.filter(b => b.paymentMethod === 'CASH').reduce((sum, b) => sum + (b.amountPaid || b.totalAmount || 0), 0);
    const todayUpi = todayBookings.filter(b => b.paymentMethod === 'UPI').reduce((sum, b) => sum + (b.amountPaid || b.totalAmount || 0), 0);
    const todayOther = todayBookings.filter(b => b.paymentMethod !== 'CASH' && b.paymentMethod !== 'UPI').reduce((sum, b) => sum + (b.amountPaid || b.totalAmount || 0), 0);
    ...
    return {
      ...
      stats: {
        ...
        attendanceRate,
        totalBookings,
        totalTicketsSold: totalTicketsSold || registered,
        totalRevenue,
        todaySales: {
      ...
      system: {
        database: 'connected (safe mode)',
        isLive: true,
        totalConfiguredSeats: 50,
        currency: 'INR'
      }
    };
  }
